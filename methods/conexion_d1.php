...
switch ($fun) {
    case 'buscarproducto':
        if (isset($_POST['val'])) {
            $query = $conexion->query("SELECT id,contratipo as text FROM producto_d1 WHERE visibilidad = 1 AND contratipo LIKE '%$_POST[val]%' OR id LIKE '%$_POST[val]%' ");
    ...
    WHERE visibilidad = 1 AND id='$id'  OR contratipo LIKE '%$id%' AND id LIKE '%$id%' ";
    ...
    case 'Q2':
        $id = $_POST['perfume'];
        $sql = "SELECT * FROM producto_d1
            WHERE id='$id'  OR contratipo LIKE '%$id%' AND id LIKE '%$id%' AND visibilidad = 1 ";
        $r = $conexion->query($sql);
        if ($o = $r->fetch_object()) {
            $resultado->resultado = $o;
        }
    ...
    case 'opciones':
      ...
      case 'buscarEnvases':
        ...
        $query = $conexion->query("SELECT * FROM producto_d1 WHERE  id_categoria=9");
