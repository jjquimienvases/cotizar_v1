...
switch ($fun) {
    case 'buscarproducto':
        if (isset($_POST['val'])) {
            $query = $conexion->query("SELECT id,contratipo as text FROM productos_ibague WHERE visibilidad = 1 AND contratipo LIKE '%$_POST[val]%' OR id LIKE '%$_POST[val]%'");
    ...
    case 'Q2':
        $id = $_POST['perfume'];
        $sql = "SELECT * FROM productos_ibague
            WHERE visibilidad != 0 AND id='$id'  OR contratipo LIKE '%$id%' AND id LIKE '%$id%' ";
        $r = $conexion->query($sql);
        if ($o = $r->fetch_object()) {
            $resultado->resultado = $o;
        }
    ...
    case 'opciones':
      ...
      case 'buscarEnvases':
        ...
        $query = $conexion->query("SELECT * FROM productos_ibague WHERE  id_categoria=9");
