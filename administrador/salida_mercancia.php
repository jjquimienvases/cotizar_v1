...
$conexion = new mysqli ('ftp.jjquimienvases.com', 'jjquimienvases_jjadmin', 'LeinerM4ster', 'jjquimienvases_cotizar'); 
 ...
 // echo "</pre>";
  ...
  echo "</pre>";
    ...
    // $consulta_datos = $conexion -> query("SELECT SUM(order_item_quantity) as total_mostrador FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden_producto.item_code LIKE $buscar_text AND factura_orden.metodopago = '$mostrador_jj' AND factura_orden.estado = '$estado'");
    $consultando_datos =$conexion ->query("SELECT SUM(order_item_quantity) AS total_mostrador FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden_producto.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.metodopago = '$mostrador_jj' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
  //mostrador principal
    $data=mysqli_fetch_assoc($consultando_datos);
    $cuentas_mostrador = $data['total_mostrador']; 
  //mostrador_d1
    $consultando_datos_d1 =$conexion ->query("SELECT SUM(order_item_quantity) AS total_d1 FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.metodopago = '$mostrador_d1' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
    $data_d1=mysqli_fetch_assoc($consultando_datos_d1);
    $cuentas_mostrador_d1 = $data_d1['total_d1']; 
//Call_Center
    $consultando_datos_call =$conexion ->query("SELECT SUM(order_item_quantity) AS total_call FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden_producto.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND user_id = $user AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
    $data_call=mysqli_fetch_assoc($consultando_datos_call);
    $cuentas_mostrador_call = $data_call['total_call']; 
    
//ENVIOS A IBAGUE     
    $consultando_datos_send_ibague =$conexion ->query("SELECT SUM(cantidad) AS total_envio_ibague FROM traspasos WHERE order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND bodega_entrada = '$ibague' AND estado LIKE '%$estado%' AND codigo = $buscar_text");
...
//CONSTAL DE  LAS COTIZACIONES 
    $consultando_nombre_producto = $conexion ->query("SELECT contratipo FROM producto_av WHERE id = $buscar_text");
...
<body>
	...
	<div class="barra__buscador">
         <div class="buscarcliente">
             <datalist id="buscarproducto">
               ...
               <?php
               $query = $conexion -> query ("SELECT * FROM producto_av");
 ...
 <ul class="list-group">
  ...
  <div id="mi_primer_resultado">
     ...
     $consulta_cot_mostrador = $conexion -> query("SELECT * FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden_producto.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.metodopago = '$mostrador_jj' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
     $consulta_cot_callcenter = $conexion -> query("SELECT * FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.user_id = '$user' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
     $consulta_cot_d1 = $conexion -> query("SELECT * FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden_producto.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.metodopago = '$mostrador_d1' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
    //  $consulta_cot_traspasos_ibague = $conexion -> query("SELECT * FROM factura_orden_producto INNER JOIN factura_orden ON factura_orden.order_id = factura_orden_producto.order_id WHERE factura_orden_producto.order_date BETWEEN '$fecha_inicial' AND '$fecha_final' AND factura_orden.metodopago = '$mostrador_jj' AND factura_orden.estado LIKE '%$estado%' AND factura_orden_producto.item_code = $buscar_text");
