...
$sel = $con->query("SELECT * FROM factura_modificada ORDER BY order_date DESC LIMIT 20");
...
if (isset($_POST['buscar_cotizacion'])) {
 $id = $_POST['producto'];
 $sql = "SELECT * FROM factura_modificada WHERE order_id='$id'  OR order_receiver_name LIKE '%$id%' ";
 $r = $con->query($sql);
 if ($o = $r->fetch_object()) {
   $resultado = $o;
 }
...
if(isset($_POST['btn_buscar'])){
  ...
  $select_buscar=$con->prepare('
    SELECT * FROM factura_modificada WHERE order_id LIKE :campo OR order_date LIKE :campo;'
  );
