...
if ($result != 0 || $result != "") {
	...
	$sql_ = $cnx->query("SELECT fa.nuevo_abono,fa.metodo_de_pago FROM file_abono fa INNER JOIN factura_orden fo ON fo.order_id = fa.order_id WHERE fa.order_id = $cotizacion ORDER BY fa.order_date DESC LIMIT 1");
	if ($sql_) {
