...
$punto_v = $_POST['metodopago'];
 ...
 $sql_get_data = $cnx->query("SELECT * FROM factura_orden_producto WHERE item_code = $item_code AND order_id = $order_id");
 ...
 $nuevo_total_items = $total_merch * $order_item_unitario;
   ...
   $sql_credito_cliente = $cnx->query("SELECT * FROM clientes WHERE cedula = $cedula");
   foreach($sql_credito_cliente as $data){
       $credito = $data['credito'];
    }
    $nuevo_credito = $credito + $total_tt;

   $sql_ver_stock = $cnx->query("SELECT * FROM $punto_venta WHERE id = $item_code");
   foreach($sql_ver_stock as $data_){
       $stock_actual = $data_['stock'];
    }
    $nuevo_stock = $stock_actual + $order_item_quantity;

   $sql_cotizacion_total = $cnx->query("SELECT * FROM factura_orden WHERE order_id = $order_id");
...
try {
    ...
    $sql_insert_devolucion = $cnx->query("INSERT INTO devolucion (order_id, cliente, cedula, item_code, item_name, item_quantity, item_total_amount, punto_venta) 
...
//   SQL PARA DAR EL SALDO A FAVOR DEL CLIENTE
    $sql_update_saldo = $cnx->query("UPDATE clientes SET credito = $nuevo_credito WHERE cedula = $cedula");
//    SQL PARA CARGAR EL INVENTARIO DE LA BODEGA EN LA QUE SE DEVOLVIO
    $sql_actualizar_stock = $cnx->query("UPDATE $punto_venta SET stock = $nuevo_stock WHERE id = $item_code");
//   SQL ACTUALIZAR TOTAL DE LA COTIZACION
    $sql_actualizar_order = $cnx->query("UPDATE factura_orden SET order_total_before_tax = $nuevo_before_tax, order_total_after_tax = $nuevo_after_tax WHERE order_id = $order_id");
   
    if($order_item_quantity == $order_quantity){
        //   SQL PARA BORRAR EL ITEM DE LA COTIZACION
        $sql_delete_item = $cnx->query("DELETE FROM factura_orden_producto WHERE order_item_id = $order_item_id");
    }else{
        //  SQL PARA ACTUALIZAR LA CANTIDAD DE ITEMS Y SU PRECIO TOTAL DEL
            $sql_update_item = $cnx->query("UPDATE factura_orden_producto SET order_item_quantity = $total_merch, order_item_final_amount = $nuevo_total_items WHERE order_item_id = $order_item_id");
    }
