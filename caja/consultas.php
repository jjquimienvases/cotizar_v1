...
function isCajaCerrada($id, $fecha = null) {
    ...
    $num = $c->findOne("SELECT * FROM finish_day WHERE DATE(order_date) = '$fechaQ' AND id_rol_usuario = $id");
...
//novedades
function subirNovedad($session, $monto, $novedad){
global $c, $soloFecha;

   $id = $session["userid"];
    $user = $session["user"];
    $punto_de_venta = getPuntoVenta($id);
    $sql = "INSERT INTO novedades_gastos (novedad, usuario, monto, user_id, punto_venta,order_date) 
    VALUES ('$novedad', '$user', $monto, $id,'$punto_de_venta','$soloFecha');";
    $result = $c->exec($sql);
}