    ...
    $(document).on("click", "#addRows", function() {
        ...
        let tr = elem("tr", {
            ...
            td: [{
                ...
                {
                    innerHTML: elem("input", {
                        attr: { placeholder: "Contratipo", size: 2, name: "productName[]", readonly:true},
                ...
                {
                    innerHTML: elem("input", {
                        attr: { placeholder: "Total", size: 2, name: "result[]", readonly:true },
...
function calculateTotal(id1) {
   ...
   var codigo = $("#productCode_" + id1).val();
      ...
      var subTotal = porcentaje + coniva;
      var total = coniva + porcentaje;
      }else if(codigo == 6963 && quantity >=11 && quantity < 12){
      var vixc = price * quantity;
      var iva = vixc * 0.19;
      var coniva = vixc + iva;
      ...
      var subTotal = porcentaje + coniva;
      var total = coniva + porcentaje;
      }else if(codigo == 6963 && quantity >=12 && quantity < 100){
    //   var vixc = price * quantity;
    //   var iva = vixc * 0.19;
    //   var coniva = vixc + iva;
    //   var porcentaje = coniva * 0.50;
    //   var subTotal = porcentaje + coniva;
      var total = 1088 * quantity;
      }else if(codigo == 6963 && quantity >=100 && quantity < 1000){
      var vixc = price * quantity;
      var iva = vixc * 0.19;
      var coniva = vixc + iva;
      ...
      var subTotal = porcentaje + coniva;
      var total = coniva + porcentaje;
      }else if(codigo == 6963 && quantity >=1000){
      var vixc = price * quantity;
      var iva = vixc * 0.19;
      var coniva = vixc + iva;
      ...
      var total = coniva + porcentaje;
            ...
            var porcentaje = coniva * 0.3;

            var subtotal = porcentaje + coniva;

            var total = subtotal / 1000;
        } else if (
            tipo_cliente == "Especiales" &&
            categoria == 4 &&
            quantity > 1
        ) {
            var vixc = price * quantity;
           ...
           tipo_cliente == "Especiales" &&
            ...
            var total = galon;
        } else if (categoria == 21 && quantity >= 1 && quantity <= 125) {
            var galon = an * quantity;

            var total = galon;
        } else if (categoria == 21 && quantity >= 125 && quantity < 250) {
            var galon = 7500;

            var total = galon;
        } else if (categoria == 21 && quantity >= 250 && quantity < 499) {
            var galon = 12000;

            var total = galon;
        } else if (categoria == 21 && quantity >= 500 && quantity < 1000) {
            var galon = 22000;
            ...
            var total = coniva + porcentaje;
        } else if (categoria != 4 && quantity <= 11) {
            //aqui empieza la formula para los demas productos
            ...
            var total = vixc;
        } else if (quantity > 0 && quantity < 63) {
            //aqui empieza la cotizacion de perfumeria
        ...
        totall = sumatotal.reduce(function(a, b) {
            return a + b;
        });
        ...
        if (sumaperfumeria != 0) {
            mytotalperfumeria = sumaperfumeria.reduce(function(a, b) {
                return a + b;
            });
        } else {
            mytotalperfumeria = 0;
   ...
   onkeydescuento_programado();
    ...
    if(desc_prom == 0){
        ...
        if (tipo_cliente == "Especiales" && categoria == 4) {
            document.getElementById("unitario_" + id1).value = (vunit / 1000).toFixed(
                2
            );
...
function getdescuento(arg) {
    ...
    if (undefined == arg.porcentaje || undefined == arg.subtotal) {
        return 0;
    }
...
/* function buscarProducto(fila) {
    var valor = $("#search1_" + fila).val();
    $.ajax({
            url: "methods/conexion_.php",
            type: "POST",
            data: { key: "Q1", producto: valor },
        })
        .done(function(d) {
            var json = JSON.parse(d);

            $("#idCategoria_" + fila).val(json.resultado.id_categoria);
            $("#productCode_" + fila).val(json.resultado.id);
            $("#productStock_" + fila).val(json.resultado.stock);
            $("#productName_" + fila).val(json.resultado.contratipo);
            $("#productUnidad_" + fila).val(json.resultado.unidad);
        })
        .fail(function(e) {
            console.log("ERROR:", e);
        });
