...
$(document).ready(function(){
            ...
            on:{
              onchange:function(evt){
                $.ajax({
                  ...
                  type:'POST',
                  dataType:'json',
                  data:{key:'Q1',producto:evt.target.selectedOptions[0].value}
                }).done(function(d){
                  console.log(d);
                  
             	let padre = $("#mibuscador").parent().parent().parent();
                  $(evt.target).parent().parent().find("[name^=idCategoria]").val(d.resultado.id_categoria)
                  $(evt.target).parent().parent().find("[name^=productCode]").val(d.resultado.id)
                  $(evt.target).parent().parent().find("[name^=productName]").val(d.resultado.contratipo)
                  $(evt.target).parent().parent().find("[name^=productStock]").val(d.resultado.stock)
                  $(evt.target).parent().parent().find("[name^=productUnidad]").val(d.resultado.unidad)
                  $(evt.target).parent().parent().find("[name^=price]").val(d.resultado.gramo)
  ...
  $(document).on('click', '#removeRows', function(){
    ...
    setTimeout(new_calculatelocal(),2000);
    });
...
var precioPerfumeria = $('#ttpes').val();
  ...
  $("[id^='result_"+id1+"']").each(function() {
    ...
    if(!quantity) {
      quantity = 1;
    }else if($(".itemRous").prop("checked") && categoria == 13 && quantity > 249){
      ...
      var subTotal = porcentaje + coniva;
      var total = coniva + porcentaje;
    }else if($(".itemjohn").prop("checked") && categoria == 4 && quantity >=1){
      var vixc = price * quantity;
      var iva = vixc * 0.19;
      var coniva = vixc + iva;
