$(document).ready(function() {
    function elem(element, args) {
        ...
        Object.entries(args).forEach(function(v, k) {
            ...
            if (["onclick", "onkeyup","onchange"].includes(v[0])) {
        ...
        switch (element) {
            case "option":
                if(undefined!==edens.elem.title){
                    edens.elem.innerHTML = edens.elem.title;
                }
                break;
            case "btngroup":
    ...
    $(document).on("click", "#Nuevo", function() {
        count++;
        let tr = elem("tr", {
            appendTo: document.querySelector("#NewItem tbody"),
 ...
 */                                     $(tr).find("[name^=codigo]").val(d.resultado.id);
                                        $(tr).find("[name^=name]").val(d.resultado.contratipo);
                                        $(tr).find("[name^=precio]").val(d.resultado.gramo);
                                        $(tr).find("[name^=stock]").val(d.resultado.stock);
                                        $(tr).find("[name^=empaque]").val(d.resultado.unidad);
                                    })
                                    .fail(function(e) {
                                        console.log("ERROR:", e);
                                    });
                ...
                //         options: [{ innerHTML: "Proveedores" }],
                //         on: {

                //             onchange: function(evt) {
                //                 console.log("leiner mena");
                //                 $.ajax({
                //                         url: "metodos/conexion_proveedor.php",
                //                         type: "POST",
                //                         data: {
                //                             key: "Q2",
                ...
                //                         var d = JSON.parse(e);
                //                         console.log(d);
                //                         $(tr).find("[name^=proveedor_name]").val(d.resultado.compa√±ia);
                //                         console.log("leiner si funciona");
                //                     })
                //                     .fail(function(e) {
                //                         console.log("ERROR:", e);
                //                         console.log("leiner no funciona");
                //                     });
                //             },
                //         },
                //         dom: function(select) {
                ...
                },
                {
                    innerHTML:elem('select',{
                        attr:{name:'bodega[]'},
                        options:[
                            {title:'Bodega Principal',value:'producto_av'},
                            {title:'Mostrador Principal',value:'producto'},
                            {title:'Mostrador D1',value:'producto_d1'},
                            {title:'IBAGUE',value:'productos_ibague'},
                        ],
                        on:{
                            onchange:function(evt){
                                console.log(evt);
...
$(document).on('click', '#removeRows', function() {
      var confirmar = confirm("è¢ƒDeseas elimiar esta fila?");
      if(confirmar == true){
        ...
        $('#checkAll').prop('checked', false);
          ...
          alert("No se elimino ninguna fila, Presiona enter");
      }
