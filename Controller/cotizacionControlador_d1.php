        ...
        if ($agregarCotizacion > 0) {
            for ($i = 0; $i < count($codigo); $i++) {
                ...
                if ($gramos[$i] != "0") {
                    ...
                    $descontar_stock = main::ejecutar_consulta_simples("UPDATE producto_d1 set stock=" . $stocknuevo[$i] . " where id='" . $codigo[$i] . "'");
                    // se esta consulatando el stock de envases
                    $consultar_gramos = main::ejecutar_consulta_simples("SELECT  * from producto_d1 where id=" . $envase[$i]);
                    while ($valor = $consultar_gramos->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $descontar_gramos = main::ejecutar_consulta_simples("UPDATE producto_d1 set stock=" . $gramos_descuento[$i] . " where id='" . $envase[$i] . "'");

                    }if (!empty($tapa[$i])) {
                        // se esta consultadando la el stock de la tapa
                        $consultar_tapas = main::ejecutar_consulta_simples("SELECT  * from producto_d1 where id=" . $tapa[$i]);
                        while ($valor = $consultar_tapas->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $descontar_gramos = main::ejecutar_consulta_simples("UPDATE producto_d1 set stock=" . $total_tapas[$i] . " where id='" . $tapa[$i] . "'");
